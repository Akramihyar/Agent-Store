...
export default function NewsletterCuratorForm({ badgeLabel = 'Newsletter Curator', agentId }: { badgeLabel?: string; agentId?: string }) {
  const [keywords, setKeywords] = useState('');
  const [sources, setSources] = useState<string[]>(['tech-news', 'industry-blogs']);
  ...
  function handleSourceChange(source: string, checked: boolean) {
    if (checked) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    appendMessage('user', `Keywords: ${keywords}\nSources: ${sources.join(', ')}\nFrequency: ${frequency}`);
    ...
    try {
      ...
      const res = await sendMessageToAgent(agent, { 
        agentId: agent.id, 
        message: `Curate ${frequency} newsletter for keywords: ${keywords} from sources: ${sources.join(', ')}`,
        metadata: payload
      });
      ...
      if (res && typeof res === 'string') {
        ...
        appendMessage('agent', `📧 Newsletter draft ready! <a href="${res.docUrl}" target="_blank" rel="noopener noreferrer" class="underline">View curated newsletter</a>`);
      } else if (res && typeof res === 'object') {
        appendMessage('agent', `Newsletter curation complete: ${JSON.stringify(res)}`);
      } else {
        appendMessage('agent', 'Newsletter has been curated successfully.');
      }
      ...
      setStatus('error');
      setErrorMsg(err?.message ?? 'Failed to curate newsletter');
    }
  }

  const availableSources = [
    { id: 'tech-news', label: 'Tech News Sites' },
    { id: 'industry-blogs', label: 'Industry Blogs' },
    { id: 'social-media', label: 'Social Media' },
    { id: 'research-papers', label: 'Research Papers' },
    { id: 'company-blogs', label: 'Company Blogs' },
    { id: 'newsletters', label: 'Other Newsletters' },
    { id: 'podcasts', label: 'Podcast Transcripts' },
  ];
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex flex-col w-full gap-3 p-3">
          <div className="text-center pb-2">
            <p className="text-sm text-muted-foreground">Curate and draft newsletters from multiple sources and keywords.</p>
          </div>
          <form onSubmit={onSubmit} className="flex flex-col w-full gap-4 p-4 border rounded-xl">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  required
                />
                <p className="text-xs text-muted-foreground">Topics to include in newsletter</p>
              </div>
              <div className="flex flex-col gap-2">
                <label htmlFor="frequency" className="text-sm font-medium">Issues per Week</label>
                <select
                  id="frequency"
                  className="w-full p-3 text-base border rounded-lg bg-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  value={frequency}
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-muted-foreground">Select content sources to curate from</p>
            </div>
            ...
            <div className="flex justify-end">
              <button
                ...
                disabled={!keywords.trim() || sources.length === 0 || status === 'sending'}
                aria-label="Curate newsletter"
              >
                <SendIcon />
                <span className="ml-2">Curate Newsletter</span>
              </button>
            </div>
